...
Magisk will now automatically invoke it's 'safe mode' after 2 bad boot attempts... If the device fails to boot normally twice in a row, Magisk will boot with Zygisk functions and all modules disabled the third time. (To facilitate this, a bootloop counter is now implemented in the magisk.db table.)
